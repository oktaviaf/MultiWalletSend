...
# Vercel supports PostgreSQL via environment variables
...
# Create tables and default tokens (only if database is configured)
if database_url:
...
def is_authenticated():
    ...
    if not database_url:
        return True  # Skip auth if no database
    ...
    try:
        ...
        logging.error(f"Authentication check error: {e}")
        return False
...
def index():
    ...
    if is_authenticated():
        ...
        return render_template('landing.html', bot_username='evmmultisender_bot')

@app.route('/login', methods=['GET', 'POST'])
def login():
    """Token-based login page"""
    if not database_url:
        # Skip login if no database configured
...
def request_token():
    ...
    if not database_url:
        flash('Fitur token tidak tersedia karena database tidak dikonfigurasi', 'warning')
        return redirect(url_for('index'))
...
def view_tokens():
    ...
    if not database_url:
        return jsonify({'error': 'Database tidak dikonfigurasi'}), 400
    ...
    try:
        ...
        logging.error(f"Error viewing tokens: {e}")
        return jsonify({'error': str(e)}), 500
